...
- `google_storage_bucket "cloudb00stabucket"`:
...
- `google_storage_bucket_iam_member "cloudb00stabucket"`:
  - `for_each = { for b in var.iam_bindings : "${b.role}-${b.member}" => b }`
  - `bucket = google_storage_bucket.cloudb00stabucket.name`, `role = each.value.role`,
